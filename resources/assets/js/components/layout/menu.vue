<template>
    <div>
        <el-menu v-if="change == 1" :default-active="current" :default-openeds="opened"
            ...
            <el-submenu v-for="v in menu" :index.sync="v.path" :router="true">
                <template slot="title"><i :class="v.icon" class="menu-icon"></i>{{v.name}}</template>
                <el-menu-item v-for="vv in v.children" :index.sync="vv.path">
        ...
        <el-menu v-if="change == 2" :default-active="current" :default-openeds="opened"
            ...
            <el-submenu v-for="v in menu" :data-menu="v.name" :index.sync="v.path"
                        class="menu_show" :router="true">
                <el-tooltip v-for="vv in v.children" class="item" effect="dark" :content="vv.name" placement="right">
                    <el-menu-item :index.sync="vv.path" style="padding-right: 18px;padding-left:18px;">
    ...
    export default{
        ...
        methods: {
            getMenu(){
                ...
                self.$http.get('/get_role_menu').then(function (res) {
                    self.menu = res.data;
