                    ...
                    <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
                        ...
                        <el-tab-pane label="关键词自动回复" name="third">
                            <div class="tab-box">
                                        ...
                                        <el-collapse-item title="新规则" name="1">
                                            <el-form :model="create" ref="create" :rules="rules" label-width="100px" style="margin-top:20px;">
                                                ...
                                                <el-form-item label="关键词">
                                                    ...
                                                    <el-tag v-for="tag in create.words" :closable="true" @close="handleClose(create,tag)" type="primary" style="margin-left:10px;">
                                                        {{tag}}
                                                    </el-tag>
                                                </el-form-item>
                                ...
                                <div style="margin-top:10px;" v-for="item in keyword">
                                    <el-collapse>
                                        <el-collapse-item :title="item.header" name="1">
                                            <el-form :model="item" :ref="item.id" :rules="rules" label-width="100px" style="margin-top:20px;">
                                                ...
                                                <el-form-item label="关键词">
                                                    ...
                                                    <el-tag v-for="tag in item.words" :closable="true" @close="handleClose(item,tag)" type="primary" style="margin-left:10px;">
                                                        {{tag}}
                                                    </el-tag>
                                                </el-form-item>
                                                <el-form-item label="回复内容">
...
<script>
    export default{
        ...
        methods: {
            ...
            getData(){
                this.loading = true;
                this.$http.post("/weixin/reply/get").then(function (response) {
                    this.loading = false;
            ...
            saveFollow() {
                this.loading = true;
                this.$http.post("/weixin/follow/set", this.follow).then(function (response) {
                    this.loading = false;
            ...
            saveNews() {
                this.loading = true;
                this.$http.post("/weixin/news/set", this.news).then(function (response) {
                    this.loading = false;
                    if(response.data.code == 0){
                        this.$message({
                            type: 'success',
            ...
            saveCreate(item,vo) {
                this.$refs[vo].validate((valid) => {
                    ...
                    if (valid) {
                        this.loading = true;
                        this.$http.post("/weixin/reply/create", item).then(function (response) {
                            this.loading = false;
                            item.validate = false;
                            item.result = {};
                            if(response.data.code == 0){
                            ...
                            if (response.data.code == 0) {
                                ...
                                item.words = [];
                                item.show = false;
                                this.keyword.splice(0,0,response.data.result);
                            }
                        });
                    }else{
            ...
            saveData(items,vo) {
                this.$refs[vo][0].validate((valid) => {
                    ...
                    if (valid) {
                        this.loading = true;
                        this.$http.post("/weixin/reply/set", items).then(function (response) {
                            this.loading = false;
                            items.btn = false;
            ...
            deleteReply(item) {
                var self = this;
                this.loading = true;
                this.$http.post("weixin/reply/delete/" + item.id).then(function (response) {
                    this.loading = false;
                    if(response.data.code == 0){
                        this.$message({
                            type: 'success',
